...
function TestSupport() {
    this.wadlDoc = null;
    this.wadlURL = '';
    this.wadlErr = '无法访问 WADL: 请重新启动 REST 风格的应用程序, 然后刷新此页面。';
    ...
    this.viewIds = [
        { "id" : "table" , "name":"表格视图", "type":"tableContent"}, 
        { "id" : "raw" , "name":"原始视图", "type":"rawContent"}, 
        { "id" : "structure" , "name":"子资源", "type":"structureInfo"},
        { "id" : "header" , "name":"头", "type":"headerInfo"},
        { "id" : "monitor" , "name":"HTTP 监视器", "type":"monitorContent"}];
...
TestSupport.prototype = {    
    ...
    initFromWadl : function () {
        ...
        if(wDoc != null) {
            ...
            this.updatepage('content', '<span class=bld>帮助页面</span><br/><br/><p>无法访问 WADL: 请重新启动 REST 应用程序, 然后刷新此页面。</p><p>如果仍然出现此错误, 并且您使用带有 Firebug 插件的 Firefox 访问此页面, <br/>则需要对本地文件禁用 Firebug。该插件位于 Firefox 菜单栏中, 请选中<br/>“工具 > Firebug > 对本地文件禁用 Firebug”</p>');
        }            
    },
    ...
    getMethodMimeTypeCombo : function (resource) {
        ...
        var str = '<table border=0><tbody><tr><td valign="top"><span id="j_id14"><label for="methodSel" class="LblLev2Txt_sun4">'+
                            '<span>选择要测试的方法: </span></label></span></td>';
        ...
        str += "</select></span></td><td width=46/><td><a class='Btn1_sun4 Btn1Hov_sun4' onclick='ts.testResource()'>测试</a></td></tr></tbody></table>";
        return str;
    },
    ...
    doShowDynamicResource : function (uri, mName, mediaType) {
        ...
        str += "</select></span></td>";
        str += "<td width=30/>"
        str += "<td><span id=j_id14><a class='Btn2_sun4 Btn1Hov_sun4' onclick='ts.addParam()'>添加参数</a>";
        str += "</span></td><td><a class='Btn1_sun4 Btn1Hov_sun4' onclick='ts.testResource()'>测试</a></td></tr></tbody></table><br/>";
        ...
        this.updatepage('request', '<span class=bld>资源:</span> '+uriLink+' <br/>(<a href="'+req+'" class=Hyp_sun4 target="_blank"><span>'+this.getDisplayURL(req, 90)+'</span></a>)');
    },
    ...
    clearAll : function() {
        ...
        this.updatepage('request', '在导航栏 (位于此页面的左侧) 上选择要测试的节点。');
    ...
    getParamRep : function (req, mName) {
        ...
        if(mName == 'PUT' || mName == 'POST') {   
            str += '<tr><td valign="top"><span id="j_id14"><label for="blobParam" class="LblLev2Txt_sun4">'+
                '<span>内容: </span></label></span></td>'+
                '<td><span id="j_id14"><textarea class="TxtAra_sun4 TxtAraVld_sun4" id=blobParam name=params rows=6 cols=65>在此处插入内容。</textarea></span></td></tr>';
        }
        str += '</tbody></table>';
        return str;
    },
    ...
    createIFrameForUrl : function (url) {
        var c = 
            '<iframe id="iFrame_" src="'+url+'" class="frame" width="'+ts.iframeWidth+'" align="left">'+
                '<p>请参见 <a class=Hyp_sun4 href="'+url+'">"'+url+'"</a>.</p>'+
            '</iframe>';
        return c;
    },
    ...
    monitor : function (method, xmlHttpReq, param) {
        ...
        var s = '请求: ' + this.currentMethod + ' ' + cURL + '\n<br/>' + params +
                    '\n\n<br/><br/>状态: ' + xmlHttpReq.status + ' (' + xmlHttpReq.statusText + ')'+
                    '\n\n<br/><br/>时间戳: ' + ' ' + xmlHttpReq.getResponseHeader('Date') + '';
        var prevs = '';
    ...
    updateContent : function (content) {
        var showRaw = true;
        var cErr = '内容可能不具有容器-包含物关系。有关内容, 请参见原始视图。';
    ...
    createTabs : function(tableContent, rawContent, showRaw) {
        ...
        if(subResources == null)
            subResources = '无可用的子资源。';
        var tabs = '<br/><span class=bld>状态:</span> '+ this.currentXmlHttpReq.status+' ('+this.currentXmlHttpReq.statusText+')<br/><br/>'+
                    '<span class=bld>响应:</span> '+
    ...
    getTab : function (id, actived) {
        ...
        if(actived)
            return '<td class="Tab1TblSelTd_sun4"><div title="当前选择: 文本字段" class="Tab1SelTxtNew_sun4"><a name="selectedTabAnchor" id="tab'+id+'"/>'+name+'</div></td>';
        else {
            if(id == '')
    ...
    debug : function(message) {
        ...
        if(dbgComp == null) {
            ...
            dbgComp.innerHTML = '<table><tr><td><span style="color: blue">Rest 调试窗口</span></td><td>'+addActionStr + '</td></tr></table><br/>';
        }
        ...
        try {
            ...
            if (this.isResponseReady(method, xmlHttpReq, content, monitor)) {
              ...
              if(rtext == undefined || rtext == '' || rtext.indexOf('HTTP Status') != -1) {
                  var err = method+' 请求Failed RequestFailed --> 状态: (' 
                  	   + xmlHttpReq.status+')\n<br/>'+
                      '响应: {<br/>' + xmlHttpReq.responseText + "<br/>}";
